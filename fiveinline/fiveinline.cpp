...
int FiveInLine::evaluateBoard(int color, std::vector<std::vector<int>>& board)
{
    int value =0;
    for(int x =0;x<FIL_COLS;x++)
    {
...
int FiveInLine::evaluateBoard(std::vector<std::vector<int>>& board)
{
...
void FiveInLine::findBestMove(int &bestX, int &bestY, int player, int depth)
{
    ...
    std::vector<std::pair<int, int>> candidates;
    std::vector<std::pair<int, int>> copyEveryStep = m_everyStepPos;
    std::vector<std::vector<int>> copyBoard = m_board;
...
void FiveInLine::getTopNMoves(int n, int player, std::vector<std::pair<int, int>>& topMoves,
                      std::vector<int>& scores)
{
    ...
    for(auto& pos : candidates){
        ...
        copyEveryStep.push_back({x, y});
        int score = minmax(copyBoard, copyEveryStep, MAX_DEPTH - 1, INT_MIN, INT_MAX, false, player);
        copyBoard[x][y] = None;
        copyEveryStep.pop_back();
...
void FiveInLine::slot_notifyAIBestMoves()
{
    ...
    if(topMoves.size() >= 1 && topMoves[0].first!=-1 && topMoves[0].second!=-1)
    {
        ...
        m_aiBest1Pos = QPoint(best1_x, best1_y);
    }
    else
    {
    ...
    if(topMoves.size() >= 2 && topMoves[1].first!=-1 && topMoves[1].second!=-1)
    {
        ...
        m_aiBest2Pos = QPoint(best2_x, best2_y);
