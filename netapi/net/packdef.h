...
enum ENUM_PLAY_ZONE{Five_In_Line = 0x10,E_L_S,D_D_Z, External_Malody = 0x20};
...
{
    STRU_JOIN_ROOM_RS():type(DEF_JOIN_ROOM_RS),userid(0),roomid(0),status(_host),result(1),inGame(0)
    {}
    PackType type;
    int userid;
    int roomid;
    int status;
    int result;
    int inGame; // 1=房间已开局（观战入局需隐藏准备/开始并同步棋盘）
}STRU_JOIN_ROOM_RS;
...
#define DEF_FIL_PIECEDOWN_RS    (_DEF_PACK_BASE + 32)

// 落子结果
enum ENUM_PIECEDOWN_RESULT {
    piecedown_fail = 0,
    piecedown_ok = 1
};
...
// 落子应答：成功仅给确认；失败带身份 + 原因，避免客户端静默丢子
struct STRU_FIL_PIECEDOWN_RS
{
    STRU_FIL_PIECEDOWN_RS()
        : type(DEF_FIL_PIECEDOWN_RS), result(piecedown_fail), userid(0), status(_spec)
        , roomid(0), color(0), x(-1), y(-1)
    {
        memset(reason, 0, sizeof(reason));
    }
    PackType type;
    int result;     // piecedown_ok / piecedown_fail
    int userid;
    int status;     // _host/_player/_spec 落子者身份
    int roomid;
    int color;
    int x;
    int y;
    char reason[_MAX_SIZE];
};
...
#define _DEF_HEARTBEAT_TIMEOUT_SEC        30    // 与服务端一致：软断(fd无效)硬离线窗口；在线则再判弱网
#define _DEF_HEARTBEAT_WEAKNET_TIMEOUT_SEC 60  // 与服务端一致：fd仍有效时延长至此再硬离线
#define _DEF_HEARTBEAT_CHECK_INTERVAL_SEC 10    // 服务端扫描周期（客户端仅作文档）
#define _DEF_NET_UNSTABLE_RTT_MS          200   // 往返时延超过则提示「网络不稳定」
...
// ========== 对手掉线通知 ==========
// kind=SOFT:   host/player 置灰；spec 不广播
// kind=HARD:   host/player 走离开房间 UI；清 room_cache；spec 广播后客户端静默 rmmem
// kind=REONLINE: host/player 恢复头像；不发 ROOM_MEMBER 避免重复 addmem
...
// 主动退出/拒绝重连：服务端按硬断清理，不可再 token 重连
#define DEF_FIL_LOGOUT_RQ               (_DEF_PACK_BASE + 31)
...
// ========== 主动注销（硬断）==========
// 客户端正常退出或拒绝断线重连时发送；服务端走 HandleDisconnect
typedef struct STRU_FIL_LOGOUT_RQ
{
    STRU_FIL_LOGOUT_RQ():type(DEF_FIL_LOGOUT_RQ),userid(0)
    {}
    PackType type;
    int userid;
}STRU_FIL_LOGOUT_RQ;
